...
{
    public class WebScrapingHavanService : IWebScrapingHavanService
    {
        private readonly IHttpClientService _httpClient;
        protected HtmlDocument currentHtml = new HtmlDocument();

        public WebScrapingHavanService(IHttpClientService httpClient)
        {
            this._httpClient = httpClient;
        }

        public async Task<ProdutoDto> ExecutarWebScrapingHavanAsync(RotaRamificadaDto rotaSementeDto)
        {
        ...
        {
            string titulo = this.currentHtml.DocumentNode.SelectSingleNode("/html/body/div[1]/main/div[2]/div/div[2]/div[1]/div[1]/h1/span").InnerText;
            string descricao = Filtros.FiltrarDescricaoHTML(this.currentHtml.DocumentNode.SelectSingleNode("/html/body/div[1]/main/div[2]/div/div[4]/div[1]/div[1]/div[2]/div/div/p").InnerText);
            float preco = Filtros.FiltrarPreco(this.currentHtml.DocumentNode.SelectSingleNode("/html/body/div[1]/main/div[2]/div/div[1]/div[2]/div[1]/span[2]/span[2]/span[2]/span").InnerText);
            float precoPromocional = Filtros.FiltrarPreco(this.currentHtml.DocumentNode.SelectSingleNode("/html/body/div[1]/main/div[2]/div/div[1]/div[2]/div[1]/span[1]/span[2]/span[2]/span").InnerText);
            float mediaAvaliacao = 5;
            string categoria = "Geladeiras";
            List<ImagemProdutoDto> imagens = ExtrairImagens();
            string sku = this.currentHtml.DocumentNode.SelectSingleNode("/html/body/div[1]/main/div[2]/div/div[2]/div[1]/div[2]/div[2]/div").InnerText;
