...
namespace WC.Rotina
{
    public class EnvioEmailServico: IHostedService, IDisposable
    {
        private Timer _timer;
        private bool disposed = false;

        private IConfiguration Configuration { get; }
        private ILogger<EnvioEmailServico> Logger { get; }
        private IServiceProvider Provider { get; }

        internal Aplicacao.ConfiguracaoServicoRotina Configuracao { get; private set; }
        internal IEnviarEmailRotinaWebScrapingAppService EnviarEmailRotinaWebScrapingAppService { get; private set; }

        public EnvioEmailServico(
            IConfiguration configuration,
            IServiceProvider provider,
            ILogger<EnvioEmailServico> logger)
        {
        ...
        private async Task ExecutarWebScrapingAsync()
        {
            ...
            {
                EnviarEmailRotinaWebScrapingAppService = scope.ServiceProvider.GetRequiredService<IEnviarEmailRotinaWebScrapingAppService>();

                if (EnviarEmailRotinaWebScrapingAppService == null)
                {
                    throw new ArgumentException($"Não foi possível recuperar '{nameof(EnviarEmailRotinaWebScrapingAppService)}'");
                }

                await EnviarEmailRotinaWebScrapingAppService.EnviarEmailRotinaWebScrapingAsync().ConfigureAwait(false);
            }
        }
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            try
            {
                Logger.LogInformation($"[{nameof(EnvioEmailServico)}] - Windows Service iniciado.");
        ...
        public Task StopAsync(CancellationToken cancellationToken)
        {
            ...
            Logger.LogInformation($"[{nameof(EnvioEmailServico)}] - Windows Service finalizado.");
